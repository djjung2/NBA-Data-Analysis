...
as.vector(names(merged_df)[2:ncol(merged_df)])

as.numeric(merged_df[1,2:ncol(merged_df)])
...
order(merged_df_rotate_ratio$wl_ratio)
...
dodge_bar <- ggplot(data = df_with_win_stat, aes(x=stat_name, y=num_games, fill=win_or_lose, order=rev(win_or_lose))) + 
  ...
  labs(title="NBA wins and losses by player statistics, 2014-2018",
       subtitle="Outcomes of games for teams with a player accomplishing various stats.",
